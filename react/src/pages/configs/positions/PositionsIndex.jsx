...
function PositionsIndex() {
  ...
  const [original, setOriginalPositions] = useState([]);
  ...
  async function getAllPositions() {
    try {
      const response = await show(
        `positions/getAllPositions?page=${currentPage}&pageSize=${pageSize}&position_name=${searchTerm}&position_status=${searchStatus}`,
      );
      if (response && response.data && response.meta) {
        const { data, meta } = response;
        setPositions(data);
        setOriginalPositions(data);
  ...
  const handleUpdate = async (id, position, status) => {
    var nstatus;
  ...
  const handleSubmit = async (valor) => {
    ...
    if (valor === 0) {
      ...
      try {
        if (current == 0) {
          response = await postWithData('positions/createPosition', dataToSend);
        } else {
          response = await putWithData(
            'positions/updatePosition/' + dataToSend.id,
            dataToSend,
          );
        }
        ToastNotify({
            ...
            <div className='mb-4 overflow-x-auto  bg-gray-50 p-2'>
              <div className='flex'>
                ...
                <select
                  ...
                  className='border rounded bg-gray-200 text-bold ml-2'
                >
          ...
          <table className='w-full bg-white border border-gray-300 mt-2'>
            <thead>
              <tr className='bg-gray-800 text-gray-100'>
                ...
                <th className='border border-gray-300'>Cargo</th>

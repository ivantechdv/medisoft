...
const FollowUps = ({ id, onFormData, onGetRecordById, setUnsavedChanges }) => {
  ...
  const getRows = async () => {
    ...
    const responseFollowUps = await getData(
      `client-follow-ups/all?employee_id=${onFormData.id}&order=${order}`,
    );
  ...
  useEffect(() => {
    const fetchSelect = async () => {
      try {
        ...
        queryParameters.append('employee_id', onFormData.id);
  ...
  const handleSubmit = async (event) => {
    ...
    const { date, title, text, client_id } = formData;

    if (
      !date ||
      !title ||
      isQuillContentEmpty(text) ||
      (selectedOption === 'client' && !client_id)
    ) {
      ToastNotify({
                    ...
                    <div className='w-full'>
                      <div className='flex justify-between items-center w-full'>
                        <h4 className='text-lg font-semibold flex-grow'>
                          {row.client_id != 0 && row.client_id != null
                            ? 'Cliente ' +
              ...
              {selectedOption === 'client' && (
                <div className='mb-4'>
                  ...
                  <Select
                    ...
                    onChange={(event) => handleChangeSelect(event, 'client_id')}
                    isSearchable
